...
Chosen: YYYYMMDD_B01 / YYYYMMDD_E01 format.
...
Chosen: New index per activation.
...
Chosen:Periodic batch version build for modifications.
...
Note2 : Further refined during v2 with Sentence boundary = period (.) only 
...
Chosen:Structured summary (Topic / Intent / Keywords).
...
Chosen:No re-ranking initially.
...
Chosen:Generation invoked only for High-confidence retrieval (≥ 0.75).
Rank-1 chunk only.
Temperature = 0.
max_tokens = 300.
