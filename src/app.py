...
# ============================================================
    ...
    new_block_text = st.text_area("Enter new rule text (under selected header)")

    # Header selection required for ADD
    st.markdown("Select Header")
    import json
    from pathlib import Path

    active_vector_path = Path("data/vector_store") / active_version / "chunk_id_to_metadata.json"

    headers = []

    if active_vector_path.exists():
        with open(active_vector_path, "r", encoding="utf-8") as f:
            metadata = json.load(f)

        headers = list(dict.fromkeys(meta["header"] for meta in metadata.values()))

    selected_header = st.selectbox("Header", headers)

    if st.button("Add to Mutation Queue"):
        if not new_block_text.strip():
            ...
            try:
                add_to_queue(
                    ...
                    chunk_id=None,
                    original_text=None,
                    proposed_text=new_block_text.strip(),
                    submitted_by="Maker"
                )

                st.success("ADD mutation queued successfully. Awaiting Checker approval.")
    ...
    if blocks:
        ...
        # Extract header from selected block
            ...
            if st.button("Submit Modification to Queue"):

                if not modified_block.strip():
                    st.warning("Modified text cannot be empty.")
                else:
                    from rag_engine.mutation_batch import add_to_queue
                    ...
                    add_to_queue(
                        ...
                        header=selected_header,
                        chunk_id=None,
                        original_text=selected_block.strip(),
                        proposed_text=cleaned_body,
                        submitted_by="Maker"
                    )

                    st.success("MODIFY mutation queued successfully. Awaiting Checker approval.")
    ...
    if blocks_for_delete:      
        ...
        if st.button("Add Delete Mutation to Queue"):
            ...
            try:
                ...
                st.success("DELETE mutation queued. Awaiting checker approval.")
...
# ============================================================
    ...
    st.divider()
    st.markdown("### Pending Mutations")

    from rag_engine.mutation_batch import remove_from_queue
    ...
    cursor.execute("""
        SELECT queue_id, mutation_type, header, proposed_text, status, original_text        
        FROM mutation_queue
    ...
    if pending:
        ...
        if st.button("Approve Batch & Build New Version"):
    ...
    cursor.execute(
        "SELECT timestamp, original_query, answer_text, success_flag, top_score "
    ...
    if logs:
        for log in logs:
            ...
            if top_score is not None:
                score_display = f"{top_score:.4f}"
                st.write(f"{timestamp} | {status} | Cosine: {score_display}")
            else:
